...
def test(args, bags_list, bag_labels, milnet):
    ...
    for i in range(0, num_bags):
        ...
        with torch.no_grad():
            ...
            # Simplified detection logic based on a single threshold
            if bag_prediction >= args.threshold:
                print(bags_list[i] + ' is detected as: Positive')
                color = [0, 1, 0]  # Green for positive
            else:
                print(bags_list[i] + ' is detected as: Negative')
                color = [1, 0, 0]  # Red for negative
            # Updating visualization logic
...
if __name__ == '__main__':
    ...
    parser.add_argument('--threshold', type=float, default=0.33796727657318115)
