...
def process_slide(args):
    img_slide, out_base, step, folder = args
    ...
    if img.properties.get('magnification') == '40':
        dimension = img.level_dimensions[2] 
    else:
    ...
    step_y_max = int(np.floor(dimension[1] / step))  # rows
    step_x_max = int(np.floor(dimension[0] / step))  # columns
    for j in range(step_y_max):  # rows
        for i in range(step_x_max):  # columns
            crop_slide(img, bag_path, (i * step, j * step), step=(j, i))
        sys.stdout.write('\r Cropped: {} -- {}/{}'.format(img_name, j + 1, step_y_max))

def slide_to_patch(out_base, img_slides, step, folder='test'):
    makedirs(out_base, exist_ok=True)
    pool = mp.Pool(mp.cpu_count())
    tasks = [(img_slide, out_base, step, folder) for img_slide in img_slides]
    for _ in tqdm(pool.imap_unordered(process_slide, tasks), total=len(tasks)):
        pass
    pool.close()
    pool.join()
